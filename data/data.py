...
def gnn_collate_fn(batch):
    """
    Collate function for GNN training — same as multiscale_collate_fn
    but also returns image_ids as a 3rd element for GNN grouping.

    Returns:
        (scale_images, labels, image_ids) — 3-tuple
    """
    scales = list(batch[0]["scales"].keys())
    scale_images = {
        scale: torch.stack([item["scales"][scale] for item in batch])
    ...
    return scale_images, labels, image_ids


class ImageGroupedBatchSampler(torch.utils.data.BatchSampler):
    """
    Batch sampler that ensures all annotations from the same image
    are in the same batch. Required for GNN to model relationships
    between specimens in the same image.

    Subclasses torch.utils.data.BatchSampler so PyTorch Lightning
    can recognize it and handle DDP properly.

    Packs complete image groups into batches up to max_batch_size.
    If a single image has more annotations than max_batch_size,
    it gets its own batch.

    For DDP: distributes whole image groups (not individual samples)
    across ranks, ensuring no image is split between GPUs.

    Args:
        image_ids: List/array of image IDs for each dataset sample
        max_batch_size: Maximum batch size
        shuffle: Whether to shuffle image groups each epoch
    ...
    def __init__(
        ...
        # Note: we don't call super().__init__() because BatchSampler
        # expects (sampler, batch_size, drop_last) but we manage our own logic.
        # We just need to quack like a BatchSampler (implement __iter__ and __len__).
        self.max_batch_size = max_batch_size
        self.batch_size = max_batch_size  # Required attribute for Lightning
        self.shuffle = shuffle
        self.drop_last = drop_last
        self.rank = rank
        self.world_size = world_size

        # Group sample indices by image_id
        groups_dict = defaultdict(list)
        for idx, img_id in enumerate(image_ids):
            groups_dict[int(img_id)].append(idx)
        all_groups = list(groups_dict.values())
        self._all_groups = all_groups  # Keep for DDP target_len computation

        # For DDP: distribute image groups across ranks
        if world_size > 1:
            # Sort groups by key for deterministic distribution
            all_groups_sorted = sorted(all_groups, key=lambda g: g[0])
            self.image_group_list = all_groups_sorted[rank::world_size]
        else:
            self.image_group_list = all_groups

        # Compute target batch count for DDP synchronization
        self._target_len = None
        self._compute_target_len()

    def _build_batches(self, groups):
        """Pack image groups into batches."""
        batches = []
        current_batch = []
        for group in groups:
            if len(current_batch) + len(group) > self.max_batch_size and current_batch:
    ...
    def __iter__(self):
        ...
        groups = list(self.image_group_list)
        if self.shuffle:
            random.shuffle(groups)

        batches = self._build_batches(groups)

        # For DDP: ensure all ranks yield the same number of batches
        # to prevent deadlocks at gradient sync barriers. Pad shorter
        # ranks by repeating batches from the beginning.
        if self.world_size > 1 and self._target_len is not None and len(batches) < self._target_len:
            while len(batches) < self._target_len:
                batches.append(batches[len(batches) % max(1, len(batches))])

        for batch in batches:
            yield batch

    def _compute_target_len(self):
        """
        For DDP, compute the max number of batches any rank would have.

        Since we can't communicate across ranks here, we compute all ranks'
        batch counts locally (deterministic given same groups and seed).
        """
        if self.world_size <= 1:
            self._target_len = None
            return

        # Rebuild what each rank would have
        max_batches = 0
        for r in range(self.world_size):
            all_groups = list(self._all_groups)
            all_groups.sort(key=lambda g: g[0])
            rank_groups = all_groups[r::self.world_size]
            rank_batches = self._build_batches(rank_groups)
            max_batches = max(max_batches, len(rank_batches))
        self._target_len = max_batches

    def __len__(self):
        if self._target_len is not None:
            return self._target_len
        total = sum(len(g) for g in self.image_group_list)
        if self.drop_last:
            return total // self.max_batch_size
        return (total + self.max_batch_size - 1) // self.max_batch_size
...
def stratified_splits(labels: List[str], cfg: DictConfig):
    ...
    Two-stage splitting:
        ...
        2. Split (val + eval) into val vs eval (if eval_ratio > 0)
    ...
    Returns:
        ...
        eval_idx: Array of evaluation indices (empty if eval_ratio=0)

    Config structure expected:
        data.train_ratio: float (e.g., 0.8)
        data.val_ratio: float (e.g., 0.2)
        data.eval_ratio: float (e.g., 0.0 for 80/20 split)
    ...
    # If no eval split, all temp goes to validation
    if eval_ratio == 0:
        return train_idx, temp_indices, np.array([], dtype=int)
