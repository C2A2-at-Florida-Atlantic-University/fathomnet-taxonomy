...
def preprocess_test_dataset(dataset_path: Path) -> Path:
    """
    Preprocesses test dataset JSON to fix null category_id values.

    For test datasets, category_id is null since labels aren't provided.
    This function replaces null values with 0 to satisfy Pydantic validation.

    Args:
        dataset_path: Path to the original dataset JSON file.

    Returns:
        Path to the preprocessed JSON file (in /tmp).
    """
    with open(dataset_path) as f:
        data = json.load(f)

    # Replace null category_id with 0 in annotations
    ...
    # Save to temp file
    import tempfile
    temp_file = Path(tempfile.mktemp(suffix=".json", prefix="fathomnet_"))
    with open(temp_file, "w") as f:
        json.dump(data, f)

    LOGGER.debug(f"Preprocessed test dataset saved to {temp_file}")
    return temp_file
...
async def download_image(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            return
        except Exception as exc:  # httpx.HTTPError and connection issues
            last_err = exc
            LOGGER.warning(
                f"[Attempt {attempt}/{max_retries}] Failed to download {url}: {exc}"
            )
...
async def process_dataset(
    ...
    """
    # Preprocess test datasets to fix null category_id
    if 'test' in dataset_path.name:
        LOGGER.info("Detected test dataset, preprocessing to fix null category_id values")
        processed_path = preprocess_test_dataset(dataset_path)
        dataset = ObjectDetectionDataset.load(processed_path)
    else:
