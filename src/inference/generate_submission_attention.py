...
from src.models.model_multiscale_attention_taxloss import MultiScaleAttentionTaxonomicClassifier
...
def main():
    ...
    # Load model - auto-detect which class to use
    print(f"Loading model from {args.checkpoint}...")

    # Try to load checkpoint to inspect model type
    checkpoint = torch.load(args.checkpoint, map_location="cpu")

    # Check if model has taxonomic loss components
    has_taxloss = any("tax_distance_loss" in k or "distance_matrix" in k
                      for k in checkpoint.get("state_dict", {}).keys())

    if has_taxloss:
        print("Detected MultiScaleAttentionTaxonomicClassifier (with taxonomic loss)")
        # Need distance matrix path for taxloss model
        distance_matrix_path = cfg.data.get("distance_matrix_csv", "data/distance_matrix.csv")
        model = MultiScaleAttentionTaxonomicClassifier.load_from_checkpoint(
            args.checkpoint,
            cfg=cfg,
            class_counts=class_counts,
            id_to_name=id_to_name,  # Required for taxloss
            scales=args.scales,
        ...
        print("Detected MultiScaleCrossAttentionClassifier (standard CE loss)")
